...
memory_add _  = do 
     ...
     if saveYesNo == "yes"
        then do putStrLn ("\n" ++ "Memory established!" ++ "\n")
                appendFile "memory.txt" (mapMemoryString ++ show (y,m,d) ++"\n")
        else do putStr "Ok. Thank You!"

memory_view _ = do
    clearScreen
    setCursorPosition 1 0
    contents     <- readFile "memory.txt"
...
memory_search _ = do
     ...
     clearScreen
     setCursorPosition 1 0
     fileExists  <- doesFileExist "memory.txt"
     if fileExists
        then do memoryLines <- readFile "memory.txt"
        ...
        then do contents <- readFile vFile
                ...
                if saveYesNo        == "yes"
                   then do putStrLn ("\n" ++ "Voice Memory established!" ++ "\n")
                           appendFile "voiceMemory.txt" (mapVoiceString ++ show (y,m,d) ++"\n")     
                else do putStr "Ok. Thank You!"
...
voice_view _ = do
    clearScreen
    setCursorPosition 1 0
    contents     <- readFile "voiceMemory.txt"
...
voice_search _ = do
     ...
     clearScreen
     setCursorPosition 1 0
     fileExists  <- doesFileExist "voiceMemory.txt"
     if fileExists
        then do memoryLines <- readFile "voiceMemory.txt"
        ...
        then do contData <- readFile hrAddFile
                ...
                if fileExists
                   then do contTime <- readFile hrAddTime
                           ...
                           setSGR [ SetColor Foreground Vivid Blue ]
                           appendFile "heartRate.txt" (hrMapList)
                           setSGR [Reset]
...
--Search Heart Rate File by Date ...EDITING HERE NOW. Fixing break when txt file doesn't exixt.
heartRate_search _ = do
     clearScreen
     setCursorPosition 1 0
     fileExists    <- doesFileExist "heartRate.txt"
     if fileExists
        then do heartRateFile <- readFile "heartRate.txt"
                ...
                let hrfSplitOn                = splitOn "-splitfile-" heartRateFile     
                    ...
                    --FIX...  THIS IS NOT WORKING. GIVING WRONG MAX AND MIN VALUES... 
                    --Likely due to NOT reading numbers as numbers and instead reading as Strings.....
                    --
                ...
                if hrDateList == []
                   ...
                   else do writeFile "temporaryWorkingHeartRate.txt" (hrDTSul)
                           setSGR [ SetColor Foreground Vivid Blue ]
